...
import {useGetAccountDetailsQuery, useUpdateAccountMutation, useDeleteAccountMutation} from "./accountApiSlice"
...
const Account = () => {
    ...
    const {
        ...
        isLoading,
        isSuccess
    } = useGetAccountDetailsQuery(params.id);
    ...
    const onSubmit = async (values, {setSubmitting}) => {
        if (mode === "VIEW_MODE") {
            ...
            const {data: id, error, invalidData} = await updateAccount(toSend);
            if (id) {
                Toast("success", "Редагування користувача пройшло успішно")
    ...
    const onUpdateBtnClick = () => {
        navigate(`/accounts/update/:${account.id}`)
    }

    const onDeleteBtnClick = async () => {
        const {data: result, error, invalidData} = await deleteAccount(account.id);
        if (result) {
            navigate(`/accounts`)
        }
    }
                                            ...
                                            <div className="form-group">
                                                <fieldset disabled={mode === "VIEW_MODE"}>
                                                    ...
                                                    <label htmlFor="lastName"
                                                           className="form-label ms-1">Прізвище</label>
                                                    <div className="input-group mb-2">
                                                        <Field type="text"
                                                ...
                                                {mode === "UPDATE_MODE" &&
                                                    <>
                                                        <label htmlFor="password"
                                                               className="form-label ms-1">Пароль</label>
                                                        <div className="input-group mb-2">
                                                            <Field type="password"
                                                        ...
                                                        <label htmlFor="confirmPassword" className="form-label ms-1">Підтвердити
                                                            пароль</label>
